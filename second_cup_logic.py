...
# # def calculate_second_cup_shifts(emp_group_full_sorted: pd.DataFrame, emp_no: str, selected_company_name: str) -> list:
# #     """
# #     Calculates shift durations for general Second Cup employees based on C/In and C/Out pairings.
# #     This logic allows shifts to span multiple calendar days.
# #     Crucially, it now only removes an initial C/Out punch if it falls between 2 AM and 7 AM.
...
# #     # Conditional removal of the first C/Out punch
...
# #                 st.write(f"DEBUG: Employee {emp_no}: Initial C/Out punch {records[0]['Original_DateTime']} ignored (within 2 AM - 7 AM window).")
# #             records = records[1:]
# #         else:
# #             if st.session_state.get('debug_mode', False):
# #                 st.write(f"DEBUG: Employee {emp_no}: Initial C/Out punch {records[0]['Original_DateTime']} NOT ignored (outside 2 AM - 7 AM window).")
...
# #                     if st.session_state.get('debug_mode', False):
# #                         st.write(f"DEBUG: Employee {emp_no}, C/In at {current_record['Original_DateTime']}: Found subsequent C/In at {next_record['Original_DateTime']} before C/Out. Treating as open shift.")
...
# #                     'Punch Status': 'Paired C/In-C/Out Shift',
...
# #                     'is_more_t_day': False,
# #                     'is_short_t_day': False,
# #                     'Punch Status': 'Open Shift (Missing C/Out)',
...
# #             if st.session_state.get('debug_mode', False):
# #                 st.write(f"DEBUG: Employee {emp_no}: Unmatched punch {current_record['Original_DateTime']} ({current_record['Status']}) ignored.")
# #             i += 1
...
# #     and DOES NOT remove an initial C/Out punch.
...
# #     # IMPORTANT: For 24-hour shifts, we DO NOT remove the first C/Out punch.
# #     # This aligns with the user's request to keep the original behavior for these specific locations.
